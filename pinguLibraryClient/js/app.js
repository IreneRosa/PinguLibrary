...
const authorName = (book) => book.authors?.map(author => `${author.name ?? ''} ${author.surname ?? ''}`.trim()).join(', ') || 'Autore non indicato';
const bookName = (book) => book?.title || 'Libro non indicato';
const showError = (target, message) => { target.textContent = message; target.classList.remove('hidden'); };
const hideError = (target) => target.classList.add('hidden');
...
function renderBooks(books = state.books) {
    ...
    if (!books.length) {
        grid.innerHTML = '<div class="empty">Nessun libro trovato. Prova a cambiare i filtri.</div>';
        return;
    }
...
function renderLoans() {
    ...
    if (!state.loans.length) {
        list.innerHTML = '<div class="empty">Nessun prestito registrato.</div>';
        return;
    }
...
function openModal(title, content, onSubmit) {
    ...
    backdrop.addEventListener('click', event => { if (event.target === backdrop)
        close(); });
    root.querySelector('form').addEventListener('submit', async (event) => { event.preventDefault(); await onSubmit(event.currentTarget); close(); });
}
function openBookModal() {
    openModal('Aggiungi un libro', `<form><div class="form-grid"><label class="field full">Titolo<input class="input" name="title" required></label><label class="field">Autore<input class="input" name="author" required></label><label class="field">Anno<input class="input" name="year" type="number"></label><label class="field">Genere<input class="input" name="genre"></label><label class="field">Lingua<input class="input" name="language" value="Italiano"></label><label class="field full">Descrizione<textarea class="input" name="description" rows="3"></textarea></label></div><div class="modal-actions"><button class="secondary" type="button" onclick="this.closest('.modal-backdrop').remove()">Annulla</button><button class="primary">Salva libro</button></div></form>`, async (form) => {
        ...
        const book = { title: String(data.get('title')), year: Number(data.get('year')) || undefined, genres: [String(data.get('genre') || '')].filter(Boolean), language: String(data.get('language') || ''), description: String(data.get('description') || ''), authors: [{ name, surname: surname.join(' ') }], availability: true };
        state.books.unshift({ ...book, id: `local-${Date.now()}` });
        renderStats();
        renderBooks();
    });
}
function openLoanModal() {
    ...
    openModal('Registra un prestito', `<form><div class="form-grid"><label class="field full">Libro<select class="input" name="book" required>${options}</select></label><label class="field">Nome lettore<input class="input" name="name" required></label><label class="field">Cognome lettore<input class="input" name="surname" required></label><label class="field">Email<input class="input" name="email" type="email"></label><label class="field">Rientro previsto<input class="input" name="endDate" type="date" required></label></div><div class="modal-actions"><button class="secondary" type="button" onclick="this.closest('.modal-backdrop').remove()">Annulla</button><button class="primary">Registra prestito</button></div></form>`, async (form) => {
        const data = new FormData(form);
        const book = state.books.find(item => (item.id ?? item.title) === data.get('book'));
        const loan = { book, user: { name: String(data.get('name')), surname: String(data.get('surname')), email: String(data.get('email') || '') }, loanStartDate: new Date().toISOString(), loanEndDate: new Date(`${data.get('endDate')}T12:00:00`).toISOString(), returned: false };
        state.loans.unshift({ ...loan, id: `local-loan-${Date.now()}` });
        if (book)
...
function setupEvents() {
    document.querySelectorAll('[data-view]').forEach(button => button.addEventListener('click', () => {
        ...
        $('#catalog-view').classList.toggle('hidden', button.dataset.view !== 'catalog');
        $('#loans-view').classList.toggle('hidden', button.dataset.view !== 'loans');
    }));
    $('#add-book').addEventListener('click', openBookModal);
    $('#add-loan').addEventListener('click', openLoanModal);
    $('#login-button').addEventListener('click', openLoginModal);
    $('#search-form').addEventListener('submit', event => { event.preventDefault(); const filters = {}; new FormData(event.currentTarget).forEach((value, key) => { if (value)
        filters[key] = String(value); }); renderBooks(state.books.filter(book => Object.values(filters).every(value => `${book.title} ${authorName(book)} ${book.year} ${book.genres?.join(' ')}`.toLowerCase().includes(value.toLowerCase())))); });
    $('#reset-search').addEventListener('click', () => { $('#search-form').reset(); renderBooks(); hideError($('#catalog-error')); });
}
setupEvents();
loadData();
export {};
